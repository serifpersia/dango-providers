...
export default function createProvider(ctx) {
  ...
  function subtitleRank(subtitle) {
    ...
    const lang = String(subtitle.language || '').toLowerCase()
    const isEn = lang === 'en' || lang.startsWith('en-') || lang.startsWith('en_') || label.includes('english')
    if (!isEn) return 5
    if (/signs?\s*&\s*songs|signs|songs/.test(label)) return 4
    if (/forced/.test(label)) return 3
    if (/\bcc\b|\bsdh\b|\bcc-|-cc/.test(label)) return 2
    if (/karaoke|styled|asakura|horriblesubs|subsplease|erai-raws/.test(label)) return 2
    return label.trim() === 'english' ? 0 : 1
  }

  function orderSubtitles(subtitles) {
    return [...subtitles]
      .map((subtitle, index) => ({ subtitle, index }))
      .sort((a, b) => subtitleRank(a.subtitle) - subtitleRank(b.subtitle) || a.index - b.index)
  ...
  async function scrapeWatch(slug, episode) {
    ...
    return {
      ...
      subtitles: orderSubtitles(
        (Array.isArray(player.subtitles) ? player.subtitles : [])
  ...
  async function mediaForTargets(targets) {
    for (const t of targets) {
      try {
        const hit = await ctx.anilist.searchByTitle(t)
        if (hit?.id) {
          const full = await getMedia(hit.id).catch(() => null)
          if (full) return full
        }
      } catch {
        // ignore
  ...
  async function resolveShowId(title, romaji) {
    try {
      ...
      const media = await mediaForTargets(targets)
      if (media) {
        const titles = buildTitles(media)
        const expected = media.episodes ?? null
        const valid = items
          ...
          .filter(Boolean)
          .sort((left, right) => right.score - left.score)
        if (valid.length > 0 && valid[0].score >= 0.8) {
          return `${NATIVE_PREFIX}${valid[0].slug}`
        }
        return null
      }
      let best = null
