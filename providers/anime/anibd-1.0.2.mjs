...
export default function createProvider(ctx) {
  ...
  function guessSubtitleLanguage(label) {
    ...
    if (l.includes('malay')) return 'ms'
    if (l.includes('hindi')) return 'hi'
    return 'en'
  }

  function extractPlayerSubtitles(html) {
    const tracks = []
    const seen = new Set()
    const objRe = /\{[^{}]*\}/g
    let obj
    while ((obj = objRe.exec(html)) !== null) {
      const block = obj[0]
      if (!/"file"\s*:/.test(block)) continue
      const label = block.match(/"label"\s*:\s*"([^"]+)"/)?.[1]?.trim() || 'English'
      ...
      tracks.push({
        label,
        language: guessSubtitleLanguage(label),
        url: file,
        isDefault: /"default"\s*:\s*true/.test(block),
      })
    }
    tracks.sort((a, b) => Number(b.isDefault) - Number(a.isDefault))
    return tracks.map(({ label, language, url }) => ({ label, language, url }))
  }
  ...
  async function resolvePlayerHls(playerLink) {
    try {
      ...
      return { hls, referer, subtitles: extractPlayerSubtitles(html) }
    } catch {
      return null
    }
  }
  ...
  async function getStreamUrls(showId, episodeNumber, mode = 'sub') {
    ...
    try {
      ...
      const candidates = groups.filter((g) => isDubGroup(g) === wantDub)
